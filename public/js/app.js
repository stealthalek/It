(() => {
  ...
  const TRANSLATIONS = {
    it: {
      ...
      nav_leave_requests: 'Ferie', leave_requests_hint: 'Richiedi ferie o permessi e tieni traccia delle tue richieste.',
      ...
      timesheet_title: 'Timbratura', timesheet_hint: 'Registra l\'inizio e la fine del tuo orario di lavoro.',
    ...
    en: {
      ...
      nav_leave_requests: 'Time off', leave_requests_hint: 'Request vacation or personal leave and track your requests.',
      ...
      timesheet_title: 'Time clock', timesheet_hint: 'Log the start and end of your work shift.',
  ...
  const NAV_KEY_BY_ROUTE = {
    dashboard: 'nav_dashboard', new: 'nav_new', search: 'nav_search', announcements: 'nav_announcements', directory: 'nav_directory', 'leave-requests': 'nav_leave_requests',
  ...
  const NAV_ICON_BY_ROUTE = {
    dashboard: 'ticket', new: 'plus', search: 'inbox', announcements: 'megaphone', directory: 'users', 'leave-requests': 'sun',
  ...
  async function renderLeaveRequests() {
    appEl.innerHTML = `
      <div class="view-header">
        <div>
          <h1>${icon('sun')} ${t('nav_leave_requests')}</h1>
          <p class="hint">${t('leave_requests_hint')}</p>
        </div>
      </div>
      <div class="card" style="margin-bottom:1.25rem">
        <h3 class="section-title" style="margin-top:0">${t('leave_new_request_title')}</h3>
      ...
      <h2 class="section-title">${t('leave_my_requests_title')}</h2>
      <div id="leaveMineWrap" class="card-list spinner-row">${t('loading')}</div>
      ${canReviewLeaveRequests() ? `
      <h2 class="section-title" style="margin-top:1.5rem">${t('leave_team_title')}</h2>
      <div class="filters">
    ...
    if (canReviewLeaveRequests()) {
      document.getElementById('leaveTeamStatusFilter').addEventListener('change', loadTeam);
      loadTeam();
    }
    loadMine();
  }
